...
void insertionSort( vector<Comparable> & a, Comparator less_than )
{
    for( int p = 1; p < a.size( ); ++p )
    {
        ...
        int j;
        for( j = p; j > 0 && less_than(tmp, a[ j - 1 ]); --j ) // added comparator
            a[ j ] = std::move( a[ j - 1 ] );
        a[ j ] = std::move( tmp );
    }
}
...
void insertionSort( vector<Comparable> & a, Comparator less_than, int left, int right )
{
    for( int p = left + 1; p <= right; ++p )
    {
        ...
        for( j = p; j > left && less_than(tmp, a[ j - 1 ]); --j ) // added comparator
            a[ j ] = std::move( a[ j - 1 ] );
        a[ j ] = std::move( tmp );
    }
}
...
void heapsort( vector<Comparable> & a , Comparator less_than)
{
    for( int i = a.size( ) / 2 - 1; i >= 0; --i )  /* buildHeap */
        percDown( a, less_than, i, a.size( ) ); // Added comparator
    for( int j = a.size( ) - 1; j > 0; --j )
    {
        std::swap( a[ 0 ], a[ j ] );               /* deleteMax */
        percDown( a, less_than, 0, j ); // Added comparator
    }
}
 ...
 */
template <typename Comparable, typename Comparator>
void merge( vector<Comparable> & a, vector<Comparable> & tmpArray, Comparator less_than,
 ...
 */
template <typename Comparable, typename Comparator>
void mergeSort( vector<Comparable> & a, vector<Comparable> & tmpArray, Comparator less_than, int left, int right )
{
    if( left < right )
    {
